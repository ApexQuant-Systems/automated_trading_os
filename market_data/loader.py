...
__spec_contract_hash__ = "0x109_production_loader"
...
class WarehouseLoader:
    ...
    def load_crypto_candles(self, symbol: str, job_id: str, candles: List[Tuple[int, float, float, float, float, float, float, int]]) -> int:
        ...
        stmt = """
            INSERT OR REPLACE INTO crypto_candles (
                timestamp, symbol, open, high, low, close, volume, quote_volume, trade_count, job_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        
        # Format rows explicitly to match our core database schema design fields
        payload = [
            (c[0], symbol, c[1], c[2], c[3], c[4], c[5], c[6], c[7], job_id)
            for c in candles
        ]
